...
В этом проекте была реализована библиотека s21_decimal.h на языке программирования Си. Эта библиотека добавила возможность работы с типом "decimal", который отсутствует в стандарте языка. Тем не менее, этот тип критически важен для, например, финансовых расчетов, где недопустимы погрешности вычислений, свойственные типам с плавающей точкой. В рамках этого проекта предполагается знакомство с задачами обработки финансовой информации, погружение в вопросы внутреннего представления различных типов данных и закрепление структурного подхода.
...
- *Если в результате операции произошло переполнение, дробная часть отбрасывается (например, 70,000,000,000,000,000,000,000,000,000 % 0.001 = 0.000)*
...
- *При обработке числа с типом float преобразовываются все содержащиеся в нём значащие десятичные цифры. Если таких цифр больше 7, то значение числа округляется к ближайшему, у которого не больше 7 значащих цифр.*
...
В процессе выполнения описанных [выше](#information) функций библиотеки были соблюдены следующие концепции:
...
- Сборка библиотеки и тестов осуществляется через Makefile (цели all, clean, test, s21_decimal.a, gcov_report)
...
- Максимальная точность составляет 28 знаков после запятой.
